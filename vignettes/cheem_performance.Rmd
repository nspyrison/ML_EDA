...
# Aside1: DALEX::plot.predict_parts() symmetrically robust?: yes
...
# Aside2: var order is different from the order of data or ordered magnetude

1) How is it defined? 
2) Can we be confident that the order from DALEX, treeshap, and fastshap will be the same?
-- it looks like treeshap::plot_contibution is in desc magnitude order, while DALEX is not.
3) Shap matrices are stored in data order, should shap order be saved? or only calculated as needed?